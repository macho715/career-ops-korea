...
* **providers:** add JobKorea Playwright-based scanner with Tailwind React SPA selectors (`providers/jobkorea.mjs`) — zero-token, headless Chromium, Korean locale context, multi-keyword search with jittered pagination
* **providers:** add Saramin official REST API provider (`providers/saramin.mjs`) — documented JSON API at `oapi.saramin.co.kr/job-search`, access-key auth, code table support for location/job-type/education, 500 req/day limit
* **apply:** add JobKorea auto-apply script (`jobkorea-apply.mjs`) — Playwright form detection with 16 field categories, Korean self-introduction/motivation essay generation from cv.md + reports, PREFLIGHT review gate, `--dry-run` preview mode, CAPTCHA detection, jobkorea-profile.yml integration
* **modes:** add `jobkorea` mode (`modes/jobkorea.md`) — unified scan → pipeline → apply workflow with Korean form field mappings (자기소개서, 지원동기, 취업우대, 개인정보동의)
